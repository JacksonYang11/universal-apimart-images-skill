...
**必须包含一张位于文章开头的插图。** 这张图的名称固定为 `opening`，`usage` 固定为 `"文章开篇插图，放置于正文开头"`，是正文的第一处插图。

**开篇插图提示词必须基于文章开头段落的内容来编写：**
1. 先阅读文章正文的前 2–4 个自然段（跳过标题和分隔符），提取核心场景、意象或情绪。
2. 提示词必须直接反映开头段落的具体内容，不得使用泛化的"文章主题插图"一类描述。
3. 提示词需用中文撰写，风格与正文章节插图一致，16:9 横幅构图。

反例（不允许）：`usage: "正文第一处插图", prompt: "一位古代谋士在烛光下沉思"`
正例（基于开头段落）：`usage: "文章开篇插图，放置于正文开头", prompt: "一个年轻人深夜在书桌前翻阅泛黄的历史穿越小说，书页泛着微光，窗外是寂静的城市夜景，桌上散落着《秦吏》《覆汉》等书籍，氛围沉静而深邃，16:9 横幅构图"`
...
- 除非用户明确要求，不要让模型生成大段说明性文字、标志、水印、复杂图表或精确 UI。

给用户确认时必须列出每张图的**完整生图提示词**（不是提示词主题或摘要），使用这种结构：
...
2. **提示词中必须包含负向约束**：每一条提示词末尾必须追加以下质量声明（脚本会自动追加，但编写提示词时也需注意不要与此冲突）：
   ```
   。注意：人物必须五官端正、手指数量正常（单手五指）、身体结构准确；器物和武器必须形态正常、无扭曲变形；画面透视和空间关系合理；画面中不要出现大段说明性文字，可有少量简洁文字点缀。
   ```
...
3. 每 60 秒批量轮询所有任务。
4. 任务完成后立即下载对应图片，记录已下载的图片编号，避免重复生成和下载。
